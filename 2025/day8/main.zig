    ...
    while (true) {
        ...
        for (0..junctions.items.len) |i| {
            for (i+1..junctions.items.len) |j| {
                ...
                const idx = std.sort.upperBound([2]usize, &sortedPairs, ctx, cmp.bound);
                if (idx >= maxConns) continue;
                @memmove(sortedPairs[idx+1..], sortedPairs[idx..maxConns-1]);
        ...
        std.mem.sort(usize, circuits.items, ctx, cmp.sort);
